...
#for i in range(run):
#    pop_easy = Population(size_pop=5000, optim="max", initial_sudoku=easy, valid_set=[i for i in range(1, 10)])
#    pop_easy.evolve_run(runs=i+1, gens=40, file_name='performance/easy_rank_swapco_mut_swap.csv', select=rank, crossover=swap_elements_co, mutate=mutation_swap, co_p = 0.90, mu_p = 0.1, elitism=True)

#run = 10 #fps
#for i in range(run):
#    pop_easy = Population(size_pop=5000, optim="max", initial_sudoku=easy, valid_set=[i for i in range(1, 10)])
#    pop_easy.evolve_run(runs=i+1, gens=40, file_name='performance/easy_fps_swapco_mut_swap.csv', select=fps, crossover=swap_elements_co, mutate=mutation_swap, co_p = 0.90, mu_p = 0.1, elitism=True)
